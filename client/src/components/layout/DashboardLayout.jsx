...
function DashboardLayoutContent() {
  ...
  useEffect(() => {
    ...
    const handleLimitReached = () => { if (endCall) endCall(); setLimitNotice(true); }, handleSidebarOpen = () => { if (window.innerWidth < 768) setIsDrawerOpen(true); else setIsCollapsed(false); };
    const handleToolCall = async (e) => {
      ...
      if (d.name === "closeModal") window.dispatchEvent(new CustomEvent("nesa:modal:close")); if (d.name === "executeLogout") handleLogout(); if (d.name === "disconnectCall" && endCall) setTimeout(() => endCall(), 1400);
